...
def Simplex(A, b, c):
    ...
    cbT, cnT, cindx, nonbasicSize = init_simplex(A,c)
    # run core simplex method until reach the optimal solution
...
A = torch.from_numpy(np.loadtxt('A.txt'))
b = torch.from_numpy(np.loadtxt('b.txt'))
