# -*- coding: utf-8 -*-
"""Копия блокнота Simplex-Matrix.ipynb with control prints

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1bvAk4NBpfhNuD0tC4kmgqtCZAOWH8vXm
"""
...
def Simplex(A, b, c):
    ...
    fname = 'cbT_' + '{:03d}'.format(n_iter) + '.out'
    np.savetxt(fname, cbT, fmt='%25.15e', delimiter='\n')

    # nonbasic variable coefficients
    cnT = np.array(c[:nonbasicSize])
    fname = 'cnT_' + '{:03d}'.format(n_iter) + '.out'
    np.savetxt(fname, cnT, fmt='%25.15e', delimiter='\n')
    ...
    while True:
        ...
        # basis matrix
        B = A[:, cbIndx]
        np.savetxt('B.out', B, fmt='%25.15e', delimiter='\n')
        Binv = np.linalg.inv(B)

        # nonbasic variable matrix
        N = A[:, cnIndx]
        np.savetxt('N.out', N, fmt='%25.15e', delimiter='\n')
